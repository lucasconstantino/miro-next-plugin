...
function bootstrap() {
  ...
  container.innerHTML = imagesSRC
    .map(src => `<img class="image-item" src="${src}" data-image-url="${src}">`)
    .join('')
  rtb.helpers.initScrollableContainerWithDraggableImages(container, {
    draggableImageSelector: '.image-item'
  })
}
